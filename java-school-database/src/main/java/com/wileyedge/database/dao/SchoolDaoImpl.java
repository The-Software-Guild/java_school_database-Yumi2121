    ...
    * Do not change any code outside of the placeholders provided.
    ...
    public List<Course> csCourses() {
        ...
        // YOUR CODE STARTS HERE
         ...
         String sql = "select c.courseCode, c.courseDesc from course c join teacher s on c.teacherId = s.tid where s.dept= 'Computer Science'";
    ...
    public List<StudentClassCount> studentsPerClass() {
        ...
        String sql = "select c.courseCode, c.courseDesc, count(e.student_id) as `numStudents` from course c left join course_student e on c.cid= e.course_id group by c.courseCode, c.courseDesc having count(e.student_id) != 0";
